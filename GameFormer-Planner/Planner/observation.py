...
def observation_adapter(history_buffer, traffic_light_data, map_api, route_roadblock_ids, device='cpu'):
    obs_len = 10  #NOTES: LED ref to train_led_trajectory_augment_input.py line 33, gameformer/data_process line 18-21
    pred_len = 40 
    _lane_len = 50
    _lane_freature = 7
    ...
    ego_agent_past_cut = data['ego_agent_past'][:obs_len, :obs_dim] #dim:(11, 7)  #NOTES: please refer to /home/arc/GameFormer-Planner/GameFormer/train_utils.py line 34 
    

    neighbor_agents_past = data['neighbor_agents_past'] #dim: (20, 11, 11)
    neighbor_agents_past_cut = data['neighbor_agents_past'][:n_neighbors, :obs_len, :obs_dim] #dim: (20, 11, 11)
    route_lanes = data['route_lanes'].unsqueeze(0) #dim: (10, 50, 3)
    map_lanes = data['map_lanes'].unsqueeze(0) #dim: (40, 50, 7)
    map_crosswalks = data['map_crosswalks'].unsqueeze(0) #dim: (5, 30, 3)
    
    past_traj = np.zeros((obs_len, 11, obs_dim)) #dim: (30, 11, 6)
    ...
    pre_motion_mask = torch.ones(11, obs_len) #dim: torch.Size([11, 10])
    # fut_motion_mask = torch.ones(11, pred_len) #dim: torch.Size([11, 20])

    data = [pre_motion_3D, pre_motion_mask, route_lanes, map_lanes, map_crosswalks, ego_agent_past, neighbor_agents_past]
    
    data = {name: value for name, value in zip(['pre_motion_3D', 'pre_motion_mask', 'route_lanes', 'map_lanes', 'map_crosswalks', 'ego_agent_past', 'neighbor_agents_past'], data)}
