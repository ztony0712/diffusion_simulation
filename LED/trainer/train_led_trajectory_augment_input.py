...
class Trainer:
	...
	def p_sample_accelerate(self, data, x, mask, cur_y, t):
		...
		# Model output
		beta = self.extract(self.betas, t.repeat(x.shape[0]), cur_y)
		# print(cur_y.shape, beta.shape, x.shape, mask.shape, eps_factor.shape, beta.shape)
		# torch.Size([44, 10, 40, 2]) torch.Size([44, 1, 1, 1]) torch.Size([44, 10, 6]) torch.Size([44, 44])
	...
	def _train_single_epoch(self, epoch):
		...
		for data in self.train_loader:
			...
			# torch.Size([44, 44]) torch.Size([44, 10, 6]) torch.Size([44, 40, 2])
			...
			# generated_y = self.p_sample_loop_accelerate(data, past_traj, traj_mask, loc, pred_len=loc.shape[1]) #[110, 10, 6], [110, 110], [110, 20, 20, 3]
			generated_y = loc
			...
			# new loss: if the ground truth is not turning or it is turning, the prediction should be the corresponding move, not a average of the two or a random move.
